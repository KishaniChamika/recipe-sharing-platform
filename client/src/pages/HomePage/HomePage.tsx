...
const HomePage: React.FC = () => {
  ...
  const handleHomeClick = (event: React.MouseEvent) => {
    if (location.pathname === '/') {
      ...
      if (homeContainer) {
        homeContainer.classList.add('blink');
        setTimeout(() => {
          homeContainer.classList.remove('blink');
        }, 100); // Duration of the blink effect
      }
    }
  };
      ...
      <div className="features">
        ...
        <div className="feature-item">
          <Link to="/categories">
            ...
            <h3>Category</h3>
            <p>Explore recipes by category for easy browsing.</p>
          </Link>
        </div>
      </div>
    </div>
